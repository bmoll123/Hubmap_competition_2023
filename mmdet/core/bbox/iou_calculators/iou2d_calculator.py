...
def bbox_overlaps(bboxes1, bboxes2, mode="iou", is_aligned=False, eps=1e-6):
    ...
    # if bboxes2.size(0) > 0:
    #     # 1. 處理 NaN / Inf
    #     bad_mask = torch.isnan(bboxes2).any(dim=1) | torch.isinf(bboxes2).any(dim=1)

    #     # 2. 處理 寬高 <= 0 的逆向框
    #     w2 = bboxes2[:, 2] - bboxes2[:, 0]
    #     h2 = bboxes2[:, 3] - bboxes2[:, 1]
    #     bad_mask = bad_mask | (w2 <= 0) | (h2 <= 0)

    #     if bad_mask.any():
    #         print(
    #             f"🛠️ [自動修復] 偵測到 {bad_mask.sum().item()} 個 RPN 幽靈壞框，正在強制重置..."
    #         )
    #         # 把所有壞框強行改成一個微小的有效正方形 [0, 0, 1, 1]，防止後面矩陣運算爆掉
    #         bboxes2[bad_mask] = bboxes2.new_tensor([0.0, 0.0, 1.0, 1.0])
    # # =========================================================
    # # ================= 抓鬼埋點結束 =================
